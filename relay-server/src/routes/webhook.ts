...
import { enqueueForward } from "../services/laravel-forward.js"
...
function pickForwardHeaders(req: Request): Record<string, string> {
  ...
  if (typeof tg === "string" && tg) out["x-telegram-bot-api-secret-token"] = tg
  const svp = req.headers["x-svp-webhook-secret"]
  if (typeof svp === "string" && svp) out["x-svp-webhook-secret"] = svp
  return out
}

function warnHostMismatch(req: Request, expectedPublicUrl: string): void {
...
webhookRouter.post("/webhook/telegram/:secret", (req: Request, res: Response) => {
...
webhookRouter.post("/webhook/telegram/reseller/:rid/:secret", (req: Request, res: Response) => {
