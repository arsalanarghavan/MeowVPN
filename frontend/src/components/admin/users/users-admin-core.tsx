...
import { UserMergeAdmin } from "@/components/admin/users/user-merge-admin"
...
export function UsersAdminCore({
  users,
  pending,
  usersPagination,
  pendingPagination,
...
usersSearchQuery: string
  ...
  const statusLabel = (st: string) => t(`status_${st}`, { defaultValue: st })
    ...
    async (userId: number, action: "approve" | "reject") => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setAlertText(parts.length ? parts.join(" — ") : t("mutateError"))
          return
        }
        onMutateSuccess?.()
      } finally {
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
            <span>
              {t("colServices")}: {formatNumber(num(u.svc_count), isFa)}
            </span>
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            {showTg ? (
            <div>
              <div className="mb-0.5 text-xs font-medium text-muted-foreground">{t("colTelegram")}</div>
              <IdsCell numericId={tg} username={uname(u)} emptyLabel="—" />
            </div>
            ) : null}
            {showBale ? (
            <div>
              <div className="mb-0.5 text-xs font-medium text-muted-foreground">{t("colBale")}</div>
  ...
  return (
    <DashPage className={"space-y-8"} data-testid="dash-users-tab">
      <DashboardPageHeader
        title={t("title")}
          ...
          <div className="flex flex-wrap gap-2">
            {canManageUsers ? (
              <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                <DialogTrigger>
                  <Button type="button" variant="default" size="sm" data-testid="dash-users-manual-create-open">
                    {t("manualCreate")}
                  </Button>
                </DialogTrigger>
                <DashDialogContent className="sm:max-w-md">
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <Label htmlFor="manual_create_username">{t("username")}</Label>
                      <Input
                        id="manual_create_username"
                        data-testid="dash-users-manual-create-username"
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="manual_create_first">{t("firstName")}</Label>
                      <Input
                        id="manual_create_first"
                        data-testid="dash-users-manual-create-first"
                    ...
                    <Button
                      type="button"
                      data-testid="dash-users-manual-create-submit"
                      disabled={createBusy || !createUsername.trim()}
                      onClick={async () => {
                        setCreateBusy(true)
                        setAlertText(null)
                        try {
                          const res = await postAdminMutate("user_manual_create", {
                            ...
                            first_name: createFirstName.trim() || createUsername.trim(),
                          })
                          if (!res.ok) {
                            setAlertText(String(res.message ?? t("actionFailed")))
                            return
                          }
                          setCreateOpen(false)
                          setCreateUsername("")
                    ...
                    >
                      {createBusy ? t("loading") : t("manualCreate")}
                    </Button>
                  </div>
                </DashDialogContent>
              </Dialog>
            ...
            <Dialog open={mergeOpen} onOpenChange={setMergeOpen}>
              ...
              <DashDialogContent className="sm:max-w-3xl">
                <DashDialogHeader>
                  <DialogTitle>{t("mergeUsers")}</DialogTitle>
                </DashDialogHeader>
                <UserMergeAdmin
        onMutateSuccess={() => {
                    onMutateSuccess?.()
      ...
      <section className="space-y-3">
        <h3 className="text-base font-medium">{t("pendingSection")}</h3>
        {pending.length === 0 ? (
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h3 className="text-base font-medium">{t("allUsersSection")}</h3>
          {usersPagination ? (
            <p className="text-xs text-muted-foreground">
              {t("listPaginationHint", { total: formatNumber(usersPagination.total, isFa) })}
            </p>
          ) : null}
        </div>
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="space-y-2">
              ...
              <DashSelect
                ...
                options={[
                  ...
                  { value: "reseller", label: t("filterRoleReseller") },
            ...
            <div className="space-y-2">
              ...
              <DashSelect
                ...
                options={[
                  { value: "all", label: t("filterAll") },
                  ...(showTg ? [{ value: "telegram", label: t("filterPlatformTelegram") }] : []),
                  ...(showBale ? [{ value: "bale", label: t("filterPlatformBale") }] : []),
                  ...(showTg && showBale ? [{ value: "both", label: t("filterPlatformBoth") }] : []),
                  { value: "none", label: t("filterPlatformNone") },
                ]}
              />
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">{t("filterSegment")}</Label>
              <DashSelect
                triggerClassName="w-auto min-w-[12rem]"
                value={listFilters.segment}
                onValueChange={(v) => onListFiltersChange({ segment: v })}
                options={[
                  { value: "all", label: t("filterSegmentAll") },
                  { value: "churned", label: t("filterSegment_churned") },
                  { value: "never_purchased", label: t("filterSegment_never_purchased") },
                  { value: "abandoned_checkout", label: t("filterSegment_abandoned_checkout") },
                  { value: "stale_buy_funnel", label: t("filterSegment_stale_buy_funnel") },
                  { value: "expiring_renew", label: t("filterSegment_expiring_renew") },
                ]}
              />
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">{t("sortLabel")}</Label>
              <DashSelect
                triggerClassName="w-auto min-w-[10rem]"
                ...
                options={[
                  ...
                  { value: "services_desc", label: t("sortServicesDesc") },
                  { value: "services_asc", label: t("sortServicesAsc") },
                  { value: "status_asc", label: t("sortStatusAsc") },
                  { value: "status_desc", label: t("sortStatusDesc") },
            ...
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">{t("svcMin")}</Label>
              <Input
                dir="ltr"
            ...
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">{t("svcMax")}</Label>
              <Input
                dir="ltr"
                className="w-28 font-mono"
        ...
        minWidth="42rem" colWidths={USERS_TABLE_COLS}>
            <thead>
              <tr className="bg-muted/40">
                ...
                <DashTh>{t("colServices")}</DashTh>
{showTg ? <DashTh>{t("colTelegram")}</DashTh> : null}
                {showBale ? <DashTh>{t("colBale")}</DashTh> : null}
