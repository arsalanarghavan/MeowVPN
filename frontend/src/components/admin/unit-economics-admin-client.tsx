...
export function UnitEconomicsAdminView({
  unitEconomics,
  panelEconomicsMap,
  panels,
  dashboardBaseUrl,
...
dashboardBaseUrl: string
  ...
  return (
    <DashPage className={"w-full space-y-6"} data-testid="dash-unit-economics-tab">
      ...
      {volumeRequired ? (
        ...
        >
          ...
          <span>{t("warnVolumeRequired")}</span>
        </div>
      ) : null}
      ...
      {lossMaking ? (
        ...
        >
          <AlertTriangle className="mt-0.5 size-4 shrink-0" aria-hidden />
          <span>{t("warnLossMaking")}</span>
        </div>
      ) : null}

      <section className="mb-8 space-y-4">
        <h2 className="text-lg font-semibold">{t("siteWideTitle")}</h2>
        <p className="text-sm text-muted-foreground">{t("liveCalcHint")}</p>
        <KpiGrid
          result={siteResult}
        currency={currency}
          tp={tp}
      ...
      <section className="mb-8 space-y-4">
        ...
        {Number(selectedPanelId) > 0 && panelResult ? (
          <>
            {sharedLines.length > 0 ? (
              <p className="text-sm text-muted-foreground">{t("sharedAllocHint")}</p>
            ) : null}
            <KpiGrid
              result={panelResult}
        currency={currency}
        ...
        {Number(selectedPanelId) > 0 && normalizedLines.length > 0 ? (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t("normalizedLinesTitle")}</CardTitle>
            </CardHeader>
            <CardContent className="overflow-x-auto">
              <table className="w-full min-w-[32rem] border-collapse text-sm">
                <thead>
                  <tr className="border-b text-muted-foreground">
                    <th className="py-2 text-start">{t("colLabel")}</th>
                    <th className="py-2 text-start">{t("colCategory")}</th>
                    <th className="py-2 text-start">{t("colCycle")}</th>
                    <th className="py-2 text-end">{t("colMonthly")}</th>
                    <th className="py-2 text-end">{t("colPerGb")}</th>
                  </tr>
                </thead>
                <tbody>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("globalInputsTitle")}</CardTitle>
          <CardDescription>{t("globalInputsDesc")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>{t("volumeSource")}</Label>
            <DashSelect
              value={globalForm.volume_mode}
              onValueChange={(v) => setGlobalForm((f) => ({ ...f, volume_mode: v }))}
              options={[
                { value: "auto_sales", label: t("volumeModeAuto") },
                { value: "manual", label: t("volumeModeManual") },
              ]}
            />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="window">{t("volumeWindowDays")}</Label>
              <Input
                id="window"
                type="number"
                min={1}
            ...
            <div className="space-y-1">
              <Label htmlFor="selling">{t("sellingPricePerGb")}</Label>
              <Input
                id="selling"
                inputMode="decimal"
          ...
          {isAutoVolume ? (
            <div className="space-y-1 rounded-md border bg-muted/30 px-3 py-2">
              <p className="text-sm font-medium">{t("salesVolumeAutoTotal")}</p>
            ...
            <div className="space-y-1">
              <Label htmlFor="volume">{t("totalVolumeGb")}</Label>
              <Input
                id="volume"
                inputMode="decimal"
          ...
          {isAutoVolume && panels.length > 0 ? (
            <div className="space-y-2">
              <p className="text-sm font-medium">{t("salesVolumeByPanel")}</p>
...
export function UnitEconomicsAdminClient() {
  ...
  return (
    <UnitEconomicsAdminView
      ...
      panelEconomicsMap={(data.panelEconomicsMap as Record<string, import("@/components/dashboard-panel-economics-sheet").PanelEconomicsEntry>) ?? {}}
