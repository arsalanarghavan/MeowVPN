...
function formFromRow(d: DashRecord): DiscountFormState {
  ...
  const restricted = num(d.restricted_user_id ?? d.restricted_svp_user_id)
  return {
    svpc_id: num(d.id),
...
function formToPayload(f: DiscountFormState): Record<string, unknown> {
  ...
  return {
    id: f.svpc_id,
    code: f.svpc_code,
    discount_type: f.svpc_type,
    discount_value: f.svpc_value,
    max_uses: f.svpc_max_uses.trim(),
    valid_from: f.svpc_valid_from.trim(),
    valid_until: f.svpc_valid_until.trim(),
    min_order_toman: f.svpc_min_order.trim(),
    max_order_toman: f.svpc_max_order.trim(),
    max_discount_toman: f.svpc_max_discount.trim(),
    restricted_user_id: restricted === "" ? 0 : num(restricted),
    allowed_plan_ids: f.svpc_allowed_plan_ids,
    active: f.svpc_active ? 1 : 0,
    allow_new_purchase: f.svpc_allow_new ? 1 : 0,
    allow_renew_same: f.svpc_allow_renew ? 1 : 0,
    allow_add_volume: f.svpc_allow_vol ? 1 : 0,
    allow_add_user_slots: f.svpc_allow_users ? 1 : 0,
  }
}
...
onMutateSuccess?: () => void
  ...
  const mutateErrorMessage = useCallback(
    (msg: string | undefined) => {
      if (msg === "plan_overlap") return t("errorPlanOverlap")
      return msg || t("mutateError")
    },
    [tp]
  )
  ...
  const onConfirmDelete = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await postAdminMutate("discount_delete", { id })
      if (!res.ok) {
        setError(mutateErrorMessage(res.message))
        return
      }
  ...
  const onToggleActive = useCallback(
    async (d: DashRecord, checked: boolean) => {
      ...
      try {
        ...
        if (!res.ok) {
          setError(res.message === "plan_overlap" ? t("errorPlanOverlap") : res.message || t("mutateError"))
          return
        }
        onMutateSuccess?.()
      } finally {
  ...
  const filterLabel =
    filter === "active" ? t("filterActive") : filter === "inactive" ? t("filterInactive") : t("filterAll")

  return (
    <DashPage className={"w-full space-y-6"} data-testid="dash-discounts-tab">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t("statsTotalRedemptions")}</CardDescription>
            <CardTitle className="text-2xl tabular-nums">{formatNumber(stats.totalRedemptions, isFa)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t("statsTotalDiscount")}</CardDescription>
      ...
      {pagination ? (
        <p className="text-xs text-muted-foreground">{t("statsPageBreakdown")}</p>
      ) : null}

      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <Label className="text-muted-foreground">{t("filterLabel")}</Label>
            <DashSelect
        ...
        saving={saving}
                tp={tp}
                typeLabel={(dtype) => t(typeLabelKey(dtype))}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <DashSheetContent className={cn("flex w-full flex-col gap-0 overflow-y-auto sm:max-w-md")}>
          <SheetHeader className="border-b p-4 text-start">
            <SheetTitle>{formMode === "add" ? t("addCode") : t("editCode")}</SheetTitle>
          </SheetHeader>
          <div className="flex-1 space-y-4 p-4">
            <div className="space-y-2">
              ...
              />
              {formMode === "edit" ? <p className="text-xs text-muted-foreground">{t("codeLockedHint")}</p> : null}
            </div>
            <div className="space-y-2">
              ...
              <DashSelect
                ...
                options={[
                  ...
                  { value: "fixed_toman", label: t("typeFixed") },
                  { value: "percent_per_gb", label: t("typePercentPerGb") },
                  { value: "fixed_per_gb", label: t("typeFixedPerGb") },
                ]}
              />
            </div>
            <div className="space-y-2">
        ...
        value={form.svpc_valid_until}
            ...
            <div className="space-y-2">
              <Label>{t("fieldMaxDiscount")}</Label>
              <Input
            ...
            <div className="space-y-2 border-t pt-2">
              <Label>{t("fieldRestrictedUser")}</Label>
              <Input
            ...
            <div className="space-y-2 border-t pt-2">
              <p className="text-xs font-medium text-muted-foreground">{t("fieldAllowedPlans")}</p>
              <p className="text-xs text-muted-foreground">{t("allowedPlansHint")}</p>
            ...
            <div className="space-y-2 border-t pt-2">
              <p className="text-xs font-medium text-muted-foreground">{t("allowSection")}</p>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  className="size-4 rounded border-input"
      ...
      <Dialog open={Boolean(usageTarget)} onOpenChange={(o) => !o && setUsageTarget(null)}>
        <DashDialogContent className={cn("max-w-lg")}>
          <DashDialogHeader className={cn("text-start")}>
            <DialogTitle>{t("usageDialogTitle")}</DialogTitle>
            <DialogDescription>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-muted-foreground">
                    <th className="py-1 text-start">{t("usageColDate")}</th>
                    <th className="py-1 text-start">{t("usageColUser")}</th>
                    <th className="py-1 text-end">{t("usageColDiscount")}</th>
                  </tr>
                </thead>
                <tbody>
                  {usageRows.map((row) => (
...
export function DiscountsAdminClient() {
  ...
  return (
    <DiscountsAdminView
      ...
      portalAdminUrl={shell?.portalAdminUrl ?? ""}
    />
  )
}
