      ...
      {!open ? (
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm font-medium">{t("viewJobReport")}</p>
            <Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)}>
              {t("hideJobReport")}
            </Button>
          </div>
          {loading ? (
            <p className="text-xs text-muted-foreground">{t("reportLoading")}</p>
          ) : items.length === 0 ? (
              ...
              <table className="w-full min-w-[32rem] text-sm">
                <thead>
                  <tr className="bg-muted/40">
                    <th className="p-2">{t("affectedUsers")}</th>
                    <th className="p-2">{t("colPanelClient")}</th>
                    <th className="p-2">{t("colItemStatus")}</th>
                    <th className="p-2">{t("colTries")}</th>
                    <th className="p-2" />
                  </tr>
                </thead>
                <tbody>
        ...
        <DashDialogContent className={cn("sm:max-w-lg")}>
          ...
          {detailItem ? (
            <ul className="space-y-2 text-sm">
              <li>
                <span className="text-muted-foreground">{t("affectedUsers")}: </span>
                <span className="font-mono">{String(detailItem.user_id ?? "—")}</span>
              </li>
              <li>
                <span className="text-muted-foreground">{t("colPanelClient")}: </span>
                {String(detailItem.client_email ?? "—")}
                {num(detailItem.panel_id) > 0 ? ` · panel ${num(detailItem.panel_id)}` : ""}
              </li>
              <li>
                <span className="text-muted-foreground">{t("colItemStatus")}: </span>
                {itemStatusLabel(String(detailItem.status ?? ""), t)}
              </li>
              <li>
                <span className="text-muted-foreground">{t("colTries")}: </span>
                {String(detailItem.tries ?? "0")}
              </li>
              {detailItem.last_error ? (
                <li>
                  <span className="text-muted-foreground">{t("colReason")}: </span>
...
export function UsersBulkAdminView({
  panels = [],
  onMutateSuccess,
...
onMutateSuccess?: () => void
  ...
  function formatDryRun(data: Record<string, unknown> | undefined) {
    ...
    if (targetCount < 1 && users < 1) {
      setDrySummary(t("dryRunEmpty"))
      return
    }
    let line =
      panelTargets > 0
        ? t("dryRunPanelSummary", {
    ...
    if (pid > 0) line += ` · ${t("dryRunSummaryPanel", { panel: pid })}`
    if (iid > 0) line += ` · ${t("dryRunSummaryInbound", { inbound: iid })}`
    setDrySummary(line)
  }
  ...
  const runProcessQueue = useCallback(async () => {
    ...
    try {
      ...
      const n = num(res.iterations)
      setProcessNotice(t("processQueueDone", { batches: String(n) }))
      await loadJobs()
      onMutateSuccess?.()
    } finally {
  ...
  const notifyMessagePlaceholder = useMemo(() => {
    if (op === "volume") return t("notifyPlaceholderVolume")
    if (op === "extend") return t("notifyPlaceholderExtend")
    if (op === "slots") return t("notifyPlaceholderSlots")
    return ""
  }, [op, tp])

  return (
    <DashPage className={"w-full space-y-8"} data-testid="dash-bulk-tab">
      <DashboardPageHeader
        title={t("title")}
        description={
          <>
            <p className="text-sm text-muted-foreground">{t("subtitle")}</p>
            <p className="mt-1 text-xs text-muted-foreground">{t("cronHint")}</p>
          </>
        }
        actions={
          canRunBulkWorker ? (
            <>
              ...
              >
                {processLoading ? t("processQueueRunning") : t("processQueueNow")}
              </Button>
          ...
          <div className="grid gap-6 xl:grid-cols-2 xl:items-start">
            <div className="space-y-4">
              <div className="space-y-3 rounded-md border border-border/60 p-3">
                <p className="text-sm font-medium">{t("scope")}</p>
                <p className="text-xs text-muted-foreground">{t("scopeHint")}</p>
                {isResellerActor ? (
                  <p className="text-xs text-amber-700 dark:text-amber-400">{t("resellerScopeHint")}</p>
                ) : null}
                <div className="space-y-2">
                  ...
                  <DashSelect
                    ...
                    disabled={busy}
                    options={[
                      { value: "all_approved", label: t("scopeAllApproved") },
                      { value: "approved_with_active_service", label: t("scopeActiveSvc") },
                      { value: "panel_active_clients", label: t("scopePanelActive") },
                      { value: "custom_ids", label: t("scopeCustom") },
                    ]}
                  />
                  {(op === "volume" || op === "extend") &&
                  (scope === "approved_with_active_service" || scope === "panel_active_clients") ? (
                    <p className="text-xs text-muted-foreground">{t("scopePanelVolumeHint")}</p>
                  ) : null}
                </div>
                {scope === "custom_ids" ? (
                  <div className="space-y-2">
                    <Label htmlFor="ids">{t("customIds")}</Label>
                    <Input
                      id="ids"
                      dir="ltr"
                      value={customIds}
                      onChange={(e) => setCustomIds(e.target.value)}
                      placeholder={t("customIdsPlaceholder")}
                      disabled={busy}
                    />
                  </div>
                ) : null}
              ...
              <div className="space-y-3 rounded-md border border-border/60 p-3">
                <p className="text-sm font-medium">{t("filterServerTitle")}</p>
                <p className="text-xs text-muted-foreground">{t("filterServerHint")}</p>
                <div className="space-y-2">
                  <Label>{t("filterPanel")}</Label>
                  <DashSelect
                    value={String(panelId)}
                    onValueChange={(v) => setPanelId(parseInt(v, 10) || 0)}
                    disabled={busy}
                    options={[
                      { value: "0", label: t("filterAllPanels") },
                      ...activePanels.map((p) => ({
              ...
              <div className="space-y-3">
                <div className="space-y-2">
                  ...
                  <DashSelect
                    ...
                    disabled={busy}
                    options={[
                      { value: "wallet", label: t("opWallet") },
                      { value: "volume", label: t("opVolume") },
                      { value: "extend", label: t("opExtend") },
                      { value: "slots", label: t("opSlots") },
                      { value: "alerts", label: t("opAlerts") },
                    ]}
                  />
                </div>
                ...
                {op === "wallet" ? (
                  <div className="space-y-2">
                    <Label htmlFor="delta">{t("delta")}</Label>
                    <Input
                      id="delta"
                      dir="ltr"
                      value={delta}
                ...
                {op === "volume" ? (
                  <div className="space-y-2">
                    <Label htmlFor="gb">{t("extraGb")}</Label>
                    <Input
                      id="gb"
                      dir="ltr"
                      value={extraGb}
                ...
                {op === "slots" ? (
                  <div className="space-y-2">
                    <Label htmlFor="slots">{t("extraUsers")}</Label>
                    <Input
                      id="slots"
                      dir="ltr"
                      value={extraUsers}
                ...
                {showServiceNotify ? (
                  <div className="space-y-3 rounded-md border border-border/60 bg-muted/20 p-3">
                    <p className="text-sm font-medium">{t("notifySection")}</p>
                    <label className={cn("flex items-center gap-2 text-sm")}>
                      <input
                        type="checkbox"
                        className="size-4 rounded border-input accent-primary"
                    ...
                    {notify ? (
                      <div className="space-y-2">
                        <Label htmlFor="notify-message">{t("notifyMessage")}</Label>
                        <Textarea
                          id="notify-message"
                          rows={4}
                          value={notifyMessage}
                        ...
                        />
                        <p className="text-xs text-muted-foreground">{t("notifyMessageHint")}</p>
                      </div>
                    ) : null}
                  </div>
                ) : null}
            ...
            <Card className="border-dashed">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">{t("previewDryRun")}</CardTitle>
              </CardHeader>
              <CardContent>
                {drySummary ? (
                  ...
                  <p className="text-sm text-muted-foreground">{t("previewDryRunEmpty")}</p>
                )}
              </CardContent>
            </Card>
          </div>
                  ...
                  <Card>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant={badgeVariant}>
                            {t(`jobStatus_${st}`, { defaultValue: t("jobStatus_unknown") })}
                          </Badge>
                        </div>
                      </div>
                    ...
                    <CardContent className="space-y-3 text-sm">
                      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        <StatBox label={t("statTotal")} value={q.total}
        />
                        <StatBox label={t("statPending")} value={q.pending}
        />
                        <StatBox label={t("statProcessing")} value={q.processing}
        />
                        <StatBox label={t("statDone")} value={q.done}
        />
                        <StatBox label={t("statFailed")} value={q.failed}
        />
                        <StatBox label={t("statSkipped")} value={q.skipped}
        />
                      </div>
