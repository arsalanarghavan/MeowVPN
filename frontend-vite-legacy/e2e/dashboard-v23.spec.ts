...
test.describe("dashboard v23 — A.1 overview", () => {
...
test.describe("dashboard v23 — A.2 monitoring", () => {
  ...
  test("poll refresh without skip", async ({ page, request }) => {
    const pollMs = process.env.NIGHTLY ? 60_000 : process.env.CI ? 5_000 : 15_000
...
test.describe("dashboard v23 — reseller nav", () => {
...
test.describe("dashboard v23 — B site_settings", () => {
...
test.describe("dashboard v23 — C users", () => {
  ...
  test("manual create via resellers mutate", async ({ request }) => {
    ...
    const res = await request.post("/api/v1/admin/mutate", {
      data: { op: "user_manual_create", username: "e2e_v23_user", first_name: "E2E" },
    })
    expect(res.ok()).toBeTruthy()
...
test.describe("dashboard v23 — D bots", () => {
  ...
  test("texts save mutate", async ({ request }) => {
    await loginAdmin(request)
    const res = await request.post("/api/v1/admin/mutate", {
      data: { op: "texts_save", key: "welcome", value: "v23" },
    })
    expect(res.ok()).toBeTruthy()
  })
...
test.describe("dashboard v23 — E panels", () => {
...
test.describe("dashboard v23 — F finance", () => {
...
test.describe("dashboard v23 — G marketing", () => {
  test("broadcast send mutate", async ({ request }) => {
    await loginAdmin(request)
    const res = await request.post("/api/v1/admin/mutate", {
      data: { op: "broadcast_send", bc_text: "v23", bc_targets: "telegram" },
    })
    expect(res.ok()).toBeTruthy()
  })
...
test.describe("dashboard v23 — H system", () => {
...
test.describe("dashboard v23 — tab markers", () => {
...
test.describe("dashboard v23 — stats alias", () => {
...
test.describe("dashboard v23 — strict UI partials", () => {
  ...
  test("users manual create from users tab", async ({ page, request }) => {
    ...
    await page.getByTestId("dash-users-manual-create-username").fill(`e2e_v23_${Date.now()}`)
...
test.describe("dashboard v23 — OPEN tab CRUD shells", () => {
  ...
  test("unit economics save panel", async ({ page, request }) => {
    ...
    await page.getByRole("button", { name: /save|ذخیره/i }).first().click()
  })
  ...
  test("broadcast send UI", async ({ page, request }) => {
    ...
    await page.getByRole("textbox").first().fill("e2e v23 broadcast")
...
test.describe("dashboard v23 — D/E depth UI", () => {
  ...
  test("configs sync and stale badge area", async ({ page, request }) => {
    ...
    await page.getByRole("button", { name: /sync|همگام/i }).first().click()
  })
...
test.describe("dashboard v23 — bot_ui and reseller panels", () => {
  test("bot_ui tab shell admin", async ({ page, request }) => {
    await loginAdmin(request)
    await page.goto("/dashboard/bot_ui/")
    await expect(page.getByTestId("dash-bot-ui-tab")).toBeVisible({ timeout: 15_000 })
  })

  test("reseller_xui_panels content tab", async ({ page, request }) => {
    await loginReseller(request)
    await page.goto("/dashboard/reseller_xui_panels/")
    await expect(page.getByTestId("dash-reseller-panels-tab")).toBeVisible({ timeout: 15_000 })
  })

  test("plans tab shell", async ({ page, request }) => {
    await loginAdmin(request)
    await page.goto("/dashboard/plans/")
    await expect(page.getByTestId("dash-plans-tab")).toBeVisible({ timeout: 15_000 })
  })
})
