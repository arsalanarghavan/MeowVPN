...
namespace App\Modules\Rial\Jobs;
...
class RialFulfillJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    public function __construct(
        public int $transactionId,
        public string $source = 'zarinpal',
    ) {}

    public function handle(PurchaseFulfillService $fulfill): void
    {
        $key = 'svp_rial_fulfill_try_'.$this->source.'_'.$this->transactionId;
        ...
        if (in_array($reason, ['bad_tx', 'no_plan_id', 'no_plan'], true)) {
            Cache::forget($key);
